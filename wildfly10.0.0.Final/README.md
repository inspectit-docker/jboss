...
```bash
$ docker run -d --link inspectIT-CMR:cmr -p 8080:8080 -v $(pwd)/config:/opt/agent/active-config inspectit/wildfly:10.0.0.Final
```
...
```bash
$ docker run -d --link inspectIT-CMR:cmr -p 8080:8080 -v $(pwd)/config:/opt/agent/active-config -e AGENT_NAME=<agent-name> inspectit/wildfly:10.0.0.Final
```

```bash
$ docker run -d --link inspectIT-CMR:cmr -h <agent-name> -p 8080:8080 -v $(pwd)/config:/opt/agent/active-config inspectit/wildfly:10.0.0.Final
```
...
```bash
$ docker build -t inspectit/wildlfy:10.0.0.Final .
```
